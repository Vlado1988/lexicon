        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h4 class="text-xl font-semibold">Edit Translation</h4>

                    <form action="{{ route('admin.translation.update', $editTranslation->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="my-2 flex items-end gap-2">
                            <div class="select_container">
                                ...
                                <select name="source_word_language" id="source_word_language" class="select_word_language border rounded w-full text-gray-900" data-word-type="source">
                                    ...
                                    <option value="{{ $language->id }}" {{ $language->id == $sourceLangId ? 'selected' : '' }}>{{ $language->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="select_container">
                                ...
                                <select name="target_word_language" id="target_word_language" class="select_word_language border rounded w-full text-gray-900" data-word-type="target">
                                    <option value="">-- Select --</option>
                                    @foreach($languages as $language)
                                    <option value="{{ $language->id }}" {{ $language->id == $targetLangId ? 'selected' : '' }}>{{ $language->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="my-2" id="source_word_container">
                            ...
                            <input type="text" name="source_word" id="source_word" class="search_word_input border rounded w-full text-gray-900" autocomplete="off" value="{{ $editTranslation->source_word }}">

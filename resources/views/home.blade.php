...
@section('content')
    <form action="{{ route('home.translate') }}" class="flex flex-wrap items-center gap-2">
        <input type="search" name="search_text" id="search-text" class="flex-shrink sm:flex-grow sm:w-auto w-full min-w-[200px]" placeholder="search ..." value="{{ request()->search_text }}">
        <div class="lang-select-container flex gap-2 items-end flex-shrink flex-wrap sm:flex-grow sm:w-fit">
            <select name="source_language" id="source_language" class="flex-1 min-w-[200px]">
            ...
            <select name="target_language" id="source_language" class="flex-1 min-w-[200px]">
                <option value="">-- Select --</option>
                @foreach ($languages as $language)
